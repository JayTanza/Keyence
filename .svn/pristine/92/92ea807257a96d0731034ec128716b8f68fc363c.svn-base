...
namespace keyencePartsVerifier.TRANSACTION
{
    public partial class actualChecking_byTables2_status : Form
    {
        ...
        int iCheckedParts = 0;
        public actualChecking_byTables2_status(string _model=null, string _line=null, string _machine=null, string _table=null, SqlConnection _con=null, string _sPeriod=null)
        {
            InitializeComponent();
            _getStatus(_model, _line, _machine, _table, _con, _sPeriod);
        }
        ...
        void _getStatus(string model, string line, string machine, string table, SqlConnection con, string period)
        {
            ...
            using (con)
            {
                ...
                while (dr.Read())
                {
                    ...
                    if (Convert.ToInt32( dr["status"].ToString().Trim() )> 0)
                    {
                        iCheckedParts++;
                    }
                }
                dr.Close();
                lbl_rows_status.Text = "Checked " + iCheckedParts.ToString() + " of " + dataGridView1.Rows.Count.ToString() + ".";
                _dgvProperty();
                statusImage();
        ...
        {
            if (Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells["dg_status"].Value.ToString()) > 0)
            {
        ...
        private void statusImage()
        {
            ...
            {
                if (Convert.ToInt32(row.Cells["dg_status"].Value.ToString()) > 0)
                {
                    Image image = new Bitmap(Properties.Resources.correct);
                    row.Cells["dg_status_image"].Value = image;


                }
                else
                {
                    Image image = new Bitmap(Properties.Resources.error);
                    row.Cells["dg_status_image"].Value = image;
