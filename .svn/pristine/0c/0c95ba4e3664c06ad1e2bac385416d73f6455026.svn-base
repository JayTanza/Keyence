...
namespace keyencePartsVerifier.MAINTENANCE
{
    public partial class feeder_parts_assign2 : Form
    {
        ...
        {
            List<tbl_web2_maintenance> list_mnt = new List<tbl_web2_maintenance>();
        ...
        {
            //var tbl = (from z in context.tbl_web2_maintenances
            //           select z).OrderBy(x=>x.modelName);
            var tbl = context.sp_get_web2_maintenance().OrderBy(x => x.modelName).ToList();
            ...
            foreach (var item in tbl)
            {
                ...
                if (!cb_line.Items.Contains(item.lineName.Trim()))
                {
                    cb_line.Items.Add(item.lineName.Trim());
                }
                if (!cb_machine.Items.Contains(item.machineName.Trim()))
                {
                    cb_machine.Items.Add(item.machineName.Trim());
                }
                if (!cb_table.Items.Contains(item.tableName.Trim()))
                {
                    cb_table.Items.Add(item.tableName.Trim());
                }
                if (!cb_parts.Items.Contains(item.partsName.Trim()))
                {
                    cb_parts.Items.Add(item.partsName.Trim());
                }
            }
