...
namespace keyencePartsVerifier.PRINTBARCODE
{
    public partial class printBarcode_feederLocation : Form
    {
        ...
        private void loadFeederLocations()
        {
            ...
            //           select new {z.mnt_id, z.order_id, z.feederLocation }).OrderBy(m=>m.mnt_id).ThenBy(m=>m.order_id).ToList().Distinct();

            var tbl = (from z in context.tbl_web2_maintenances
                       select new { z.feederLocation }).OrderBy(m => m.feederLocation).ToList().Distinct();
            ...
            //data.Rows.Add(0, "12345678910abcdefg");
        }
