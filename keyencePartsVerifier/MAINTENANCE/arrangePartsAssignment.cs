...
namespace keyencePartsVerifier.MAINTENANCE
{
    public partial class arrangePartsAssignment : Form
    {
        ...
        private void _filterData(object sender1, EventArgs e1)
        {
            if (isLoaded)
            {
                var tbl = (from z in context.tbl_web2_maintenances
                           ...
                           select new {z.order_id, z.mnt_id, z.lineName, z.machineName, z.tableName, z.modelName, z.feederLocation, z.partsName }
                    //select z, x
                ).OrderBy(m => m.order_id).ToList();
                data.Rows.Clear();
                for (int i = 0; i < tbl.Count; i++)
                {
                    data.Rows.Add(tbl[i].mnt_id,tbl[i].order_id, tbl[i].modelName, tbl[i].lineName, tbl[i].machineName, tbl[i].tableName, tbl[i].feederLocation, tbl[i].partsName);
                }
            }
        }
        ...
        void _saveWithOrderID(int mnt_id, int order_id)
        {
            tbl_web2_maintenance mnt = (from z in context.tbl_web2_maintenances
                                        where z.mnt_id == mnt_id
                                        select z).SingleOrDefault();
            mnt.order_id = order_id;
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            if (data.Rows.Count > 0)
            {
                ...
                {
                    _saveWithOrderID(Convert.ToInt32( data.Rows[i].Cells["dg_mnt_id"].Value), i+1);
                    data.Rows[i].Cells["dg_order_id"].Value = i + 1;
                }

                MessageBox.Show("Saved feeder & parts arrangement!", "Saved Arrangement", MessageBoxButtons.OK,MessageBoxIcon.Information);
            }
        }
